	...
	render_layers() {
		...
		if (config.layer) {
			for (var i in layers) {
				...
				var thumbnail = '';
				if (value.type === 'image' && value.link && value.link.src) {
					thumbnail = '<span class="layer_thumbnail"><img alt="" src="' + this.Helper.escapeHtml(value.link.src) + '" /></span>';
				}
				
				html += '	<button class="layer_name" id="layer_name" data-id="' + value.id + '">' + thumbnail + '<span class="layer_label">' + layer_title + '</span></button>';
