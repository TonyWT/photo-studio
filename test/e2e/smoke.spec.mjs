...
test('主页提供本地打开和新建入口', async ({ page }) => {
  ...
  await expect(page.getByTestId('dropzone')).toContainText('paste an image');
  await expect(page.getByTestId('recent-projects')).toContainText('No saved local projects yet');
});
...
test('状态栏以单一保存入口收束本地保存与导出操作', async ({ page }) => {
  ...
  await expect(page.getByTestId('editor-close')).toHaveText('Close');
  await expect(page.getByTestId('save-menu-toggle')).toHaveText('Save');
  await expect(page.locator('[data-editor-zoom="out"] img')).toHaveAttribute('src', /\/images\/icons\/magnifying-glass-minus\.svg$/);
  await expect(page.locator('[data-editor-zoom="in"] img')).toHaveAttribute('src', /\/images\/icons\/magnifying-glass-plus\.svg$/);
...
test('编辑器提供 Pixlr 风格的非 AI 工作台骨架', async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => {
    ...
    return [Math.round(panel.getBoundingClientRect().width), Math.round(workspace.getBoundingClientRect().left)];
  })).toEqual([324, 384]);
...
test('手动 Cutout 的套索会画出绿色选框，闭合后即时抠图，撤销可恢复预览前画面', async ({ page }) => {
  await openHome(page);
  await page.getByTestId('image-picker').setInputFiles(filterPixelFixture);
  await expect(page).toHaveURL(/\/editor\/$/);
  await expect.poll(() => page.evaluate(() => Boolean(window.PhotoStudio))).toBe(true);
  ...
  await page.mouse.down();
  await page.mouse.move(bounds.x + bounds.width * 0.52, bounds.y + bounds.height * 0.26);
  await page.mouse.move(bounds.x + bounds.width * 0.52, bounds.y + bounds.height * 0.52);
  await page.mouse.move(bounds.x + bounds.width * 0.28, bounds.y + bounds.height * 0.52);
  await page.mouse.up();
  ...
  await expect.poll(() => page.evaluate(() => {
    ...
    if (!(overlay instanceof HTMLCanvasElement)) return { green: 0, previewOutside: 255 };
    ...
    const preview = window.AppConfig.layer.link_canvas.getContext('2d').getImageData(2, 2, 1, 1).data[3];
    return { green, previewOutside: preview };
  })).toEqual(expect.objectContaining({ previewOutside: 0 }));
  const overlayGreen = await page.evaluate(() => {
    const overlay = document.querySelector('[data-testid="cutout-path-overlay"]');
    const data = overlay.getContext('2d').getImageData(0, 0, overlay.width, overlay.height).data;
    let green = 0;
    for (let index = 0; index < data.length; index += 4) {
  ...
  expect(overlayGreen).toBeGreaterThan(0);
  await expect.poll(() => page.evaluate(() => window.State.action_history.length)).toBe(historyBefore);

  await page.locator('[data-editor-history="undo"]').click();
  await expect.poll(() => page.evaluate(() => window.PhotoStudio.getCutoutSelection().regions.length)).toBe(0);
  await expect.poll(() => page.evaluate(() => window.AppConfig.layer.link_canvas == null)).toBe(true);
  await expect(page.getByTestId('cutout-path-overlay')).toHaveCount(0);
  await expect.poll(() => page.evaluate(() => {
    const image = window.AppConfig.layer.link;
    const scratch = document.createElement('canvas');
    scratch.width = image.naturalWidth;
...
test('Retouch 首屏按参考的 Repair、Clone、Detail、Toning 四类本地工作流排序', async ({ page }) => {
  ...
  await expect(page.getByTestId('retouch-method-card')).toBeVisible();
  await expect(page.getByTestId('retouch-method-card').locator('strong')).toHaveText('Method');
  await expect(page.getByTestId('retouch-size')).toBeVisible();
  await expect(page.getByTestId('retouch-object')).toHaveCount(0);
});
...
test('Liquify 面板回写本地内核参数，并按 WebGL2 与锁定状态安全执行', async ({ page }) => {
  ...
  await expect(page.getByTestId('liquify-radius').locator('xpath=..')).toContainText('Size');
  ...
  await expect(page.getByTestId('liquify-strength').locator('xpath=..')).toContainText('Strength');
  await expect(page.getByTestId('liquify-density')).toHaveAttribute('min', '1');
  await expect(page.getByTestId('liquify-density').locator('xpath=..')).toContainText('Density');
  await expect(page.getByTestId('liquify-high-quality')).not.toBeChecked();
  await expect(page.getByTestId('liquify-high-quality').locator('xpath=..')).toContainText('High quality preview');
...
test('Drawing 橡皮可以擦掉刚画上的笔触', async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => window.State.action_history.length)).toBeGreaterThanOrEqual(history + 2);
  await expect.poll(displayPixel).not.toEqual([217, 70, 239, 255]);
  await expect.poll(activeLayerPixel).toEqual([0, 0, 0, 0]);
});

test('Drawing 柔化橡皮拖动应连续擦掉笔触，而不是留下半透明圆戳', async ({ page }) => {
  await openHome(page);
  const drawingFixture = await page.evaluate(() => {
    const canvas = document.createElement('canvas');
    canvas.width = 270;
  ...
  await page.getByTestId('tool-drawing').click();
  await expect(page.getByTestId('drawing-eraser')).toBeVisible();
  await expect(page.locator('#canvas_minipaint_background')).toHaveClass(/squares/);
  await expect(page.locator('#canvas_minipaint_background')).not.toHaveClass(/white/);
  await page.getByTestId('drawing-color').fill('#d946ef');
  await page.getByTestId('drawing-size').fill('18');
  await page.getByTestId('drawing-softness').fill('20');
  await page.getByTestId('drawing-opacity').fill('100');
  await page.getByTestId('drawing-brush').click();

  const layerPixel = (x, y) => page.evaluate(({ sampleX, sampleY }) => {
    const layer = window.AppConfig.layer;
    const source = layer.link_canvas || layer.link;
    const canvas = document.createElement('canvas');
    canvas.width = layer.width_original || source.width || source.naturalWidth;
  ...
  const canvasBox = await canvas.boundingBox();
  expect(canvasBox).not.toBeNull();
  await canvas.hover({ position: { x: 70, y: 135 } });
  await page.mouse.down();
  await page.mouse.move(canvasBox.x + 200, canvasBox.y + 135, { steps: 12 });
  await page.mouse.up();
  await expect.poll(() => layerPixel(140, 135)).toEqual([217, 70, 239, 255]);

  await page.getByTestId('drawing-size').fill('28');
  await page.getByTestId('drawing-eraser').click();
  await expect.poll(() => page.evaluate(() => window.AppConfig.TOOL.name)).toBe('erase');
  await canvas.hover({ position: { x: 70, y: 135 } });
  await page.mouse.down();
  await page.mouse.move(canvasBox.x + 200, canvasBox.y + 135, { steps: 1 });
  await page.mouse.up();

  await expect.poll(() => layerPixel(140, 135).then((pixel) => pixel[3])).toBe(0);
  await expect.poll(() => layerPixel(140, 135)).not.toEqual([217, 70, 239, 255]);
  await expect.poll(() => layerPixel(140, 135)).not.toEqual([255, 255, 255, 255]);
});

test('Drawing 选中橡皮后预览呈现透明挖空而不是前景色', async ({ page }) => {
  await openHome(page);
  await page.getByTestId('image-picker').setInputFiles({ name: 'erase-preview.png', mimeType: 'image/png', buffer: samplePng });
  await expect(page).toHaveURL(/\/editor\/$/);
  await expect.poll(() => page.evaluate(() => Boolean(window.PhotoStudio?.activateEditorTool))).toBe(true);
  await page.getByTestId('tool-drawing').click();
  await expect(page.getByTestId('drawing-eraser')).toBeVisible();
  ...
  await page.getByTestId('drawing-eraser').click();
  await expect(page.locator('#tools_container .erase')).toHaveClass(/active/);
  await expect(page.getByTestId('drawing-eraser')).toHaveClass(/is-selected/);
  const preview = await page.evaluate(() => {
    const canvas = document.querySelector('[data-testid="drawing-brush-preview"]');
    const context = canvas.getContext('2d', { willReadFrequently: true });
    return Array.from(context.getImageData(canvas.width / 2, canvas.height / 2, 1, 1).data);
  });
  expect(preview.slice(0, 3)).not.toEqual([217, 70, 239]);
  expect(preview[0]).toBeGreaterThan(40);
  expect(preview[0]).toBeLessThan(90);
});

test('Drawing 柔化会改变橡皮外缘透明度', async ({ page }) => {
  await openHome(page);
  const drawingFixture = await page.evaluate(() => {
    const canvas = document.createElement('canvas');
    canvas.width = 270;
  ...
  await expect.poll(() => page.evaluate(() => window.AppConfig.TOOL.name)).toBe('erase');
  await expect.poll(() => page.evaluate(() => (
    window.AppConfig.TOOLS.find((tool) => tool.name === 'erase').attributes.softness
  ))).toBe(20);

  const edgeAlpha = () => page.evaluate(() => {
    const canvas = document.getElementById('canvas_minipaint');
    return canvas.getContext('2d', { willReadFrequently: true }).getImageData(148, 135, 1, 1).data[3];
  });
  await page.getByTestId('drawing-softness').fill('0');
  await expect.poll(() => page.evaluate(() => (
    window.AppConfig.TOOLS.find((tool) => tool.name === 'erase').attributes.softness
  ...
  await page.locator('#canvas_minipaint').click({ position: { x: 134, y: 134 } });
  const hardEdge = await edgeAlpha();
  await page.locator('[data-editor-history="undo"]').click();
  await expect.poll(edgeAlpha).toBe(255);
  ...
  await page.locator('#canvas_minipaint').click({ position: { x: 134, y: 134 } });
  await expect.poll(edgeAlpha).not.toBe(hardEdge);
  await expect.poll(edgeAlpha).toBeLessThan(255);
});

test('Drawing 在图层类型尚未写回 image 时仍能擦掉可见笔触', async ({ page }) => {
  await openHome(page);
  const drawingFixture = await page.evaluate(() => {
    const canvas = document.createElement('canvas');
    canvas.width = 270;
  ...
  await expect.poll(() => page.evaluate(() => {
    const canvas = document.getElementById('canvas_minipaint');
    return Array.from(canvas.getContext('2d', { willReadFrequently: true }).getImageData(135, 135, 1, 1).data);
  })).not.toEqual([20, 30, 40, 255]);

  await page.evaluate(() => {
    window.AppConfig.layer.type = null;
  });
  await page.getByTestId('drawing-eraser').click();
  await page.locator('#canvas_minipaint').click({ position: { x: 134, y: 134 } });
  await expect.poll(() => page.evaluate(() => {
...
test('编辑器可保存本地项目', async ({ page }) => {
  ...
  await expect(page.getByTestId('save-local-project')).toHaveText('Project saved');
});
...
test('最近项目展示缩略图、名称和时间，并支持删除本地项目', async ({ page }) => {
  await openHome(page);
  await page.getByTestId('image-picker').setInputFiles({
    name: 'recent-delete.png',
    mimeType: 'image/png',
    buffer: samplePng,
  });
  await expect(page).toHaveURL(/\/editor\/$/);
  ...
  await page.getByTestId('editor-home').click();
  await expect(page.locator('[data-page="home"]')).toHaveAttribute('data-ready', 'true');
  const card = page.locator('[data-testid="recent-projects"] .project-card').first();
  await expect(card).toBeVisible();
  await expect(card.locator('img')).toHaveAttribute('src', /^data:image\//);
  await expect(card.locator('strong')).not.toHaveText('');
  await expect(card.locator('span')).toHaveText(/\d/);
