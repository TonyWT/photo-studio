...
test('Effect 的预览取消不写历史，确认后可撤销；锁定层不能通过动作绕过', async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => window.State.action_history_index)).toBe(before.index + 1);
  await page.locator('[data-editor-history="undo"]').click();
  ...
  const actionStatus = await page.evaluate(async (layerId) => {
    const result = await window.State.do_action(new window.app.Actions.Add_layer_filter_action(layerId, 'contrast', { value: 25 }));
...
test('Effect 更新已有实时效果时撤销会恢复原有参数', async ({ page }) => {
  ...
  const filter = await page.evaluate(async () => {
    await window.State.do_action(new window.app.Actions.Add_layer_filter_action(null, 'contrast', { value: 20 }));
    return { ...window.AppConfig.layer.filters[0], params: { ...window.AppConfig.layer.filters[0].params } };
  });
  await page.evaluate(async (current) => {
    await window.State.do_action(new window.app.Actions.Add_layer_filter_action(null, 'contrast', { value: 70 }, current.id));
  }, filter);
